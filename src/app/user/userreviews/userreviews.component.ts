...
export class UserreviewsComponent implements OnInit {
  ...
  profileData() {
    ...
    this.userdataservice
      .getProfile(this.profile.id, this.profile.token)
      .subscribe((res) => {
        if (res.status == 0) {
          this.session.logout();
          this.router.navigate(['login']);
  ...
  editReview(i: any, review: any) 
  {
      ...
      this.modalService.open(review, { windowClass: 'mobile-modalbox' });
  }
  ...
  onSubmit() {
    ...
    if (this.reviewForm.invalid) {
      ...
      this.userdataservice
        .updateReview(reviewData, this.reviewdata.id)
        .subscribe(
          (res) => {
            if (res.status == 1) {
              this.allReview();
  ...
  deleteReview(i: any) 
  {
    ...
    {
        this.userdataservice
        .deleteReview(this.reviewdata.id, this.profile.id)
        .subscribe(
          (res) => {
            if (res.status == 1) {
              this.allReview();
  ...
  allReview() 
  {
    ...
    this.spinner.show();
    this.userdataservice
      .UserReviews(this.user.id, this.user.token)
      .subscribe((res) => {
